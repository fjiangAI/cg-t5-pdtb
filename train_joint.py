...
from DS.data_set_joint import CustomDataSet, collate_func
        ...
        for step, batch in enumerate(iter_bar):
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.eval_steps > 0 and global_step % args.eval_steps == 0:
                    ...
                    model.train()
        # 每个epoch进行完，则保存模型
        output_dir = os.path.join(args.output_dir, "model-{}".format(9))
...
def set_args():
    ...
    parser.add_argument('--config_path', default='./t5/config.json', type=str, help='模型参数配置信息')
    parser.add_argument('--vocab_path', default='./t5/vocab.txt', type=str, help='词表，该词表为小词表，并增加了一些新的标记')
    parser.add_argument('--train_file_path', default='./data_dir/train_data.json', type=str, help='训练数据')
    parser.add_argument('--test_file_path', default='./data_dir/test_data.json', type=str, help='测试数据')
    parser.add_argument('--pretrained_model_path', default='./t5-base/', type=str, help='预训练模型的路径')
    ...
    parser.add_argument('--logging_steps', default=150, type=int, help='保存训练日志的步数')
    parser.add_argument('--eval_steps', default=1551, type=int, help='训练时，多少步进行一次测试')
    ...
    parser.add_argument('--title_max_len', type=int, default=200, help='生成的最大长度，要比max_len小')
    parser.add_argument('--class_size', type=int, default=11, help='类别')
...
def main():
    ...
    # 加载训练数据和测试数据
    train_data = CustomDataSet(tokenizer, args.max_len, args.title_max_len, args.data_dir, "train",
                               args.train_file_path, is_overwrite=True)
    test_data = CustomDataSet(tokenizer, args.max_len, args.title_max_len, args.data_dir, "test",
